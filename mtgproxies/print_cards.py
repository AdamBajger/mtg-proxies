...
def blend_patch_into_image(bbox: tuple[int, int, int, int], image: PIL.Image, patch_inverted: PIL.Image):
    size = min(image.size)
    alpha = image.split()[3]
    alpha = ImageChops.invert(alpha)
    dill_size = size // 10 + 1 - (size // 10 % 2)
    blur_size = size // 30
    ...
    image.paste(patch_inverted, mask=alpha.crop(bbox), box=bbox)
    return image


def fill_transparent_corner_vertical_left(
    corner_square: NDArray[Shape[Any, Any, 4], Float], thinning_factor: int = 6
) -> NDArray[Shape[2], Float]:
    corner_copy = corner_square.copy()
    width, height = corner_copy.shape[:2]
    bbox = (0, 0, width // thinning_factor, height)

    image = PIL.Image.fromarray(corner_copy)
    patch_inverted = image.crop(bbox).transpose(method=Transpose.FLIP_TOP_BOTTOM)
    return blend_patch_into_image(bbox, image, patch_inverted)


def fill_transparent_corner_vertical_right(
    corner_square: NDArray[Shape[Any, Any, 4], Float], thinning_factor: int = 6
) -> NDArray[Shape[2], Float]:
    corner_copy = corner_square.copy()
    width, height = corner_copy.shape[:2]
    bbox = ((thinning_factor - 1) * width // thinning_factor, 0, width, height)
...
def fill_transparent_corner_horizontal_top(
    corner_square: NDArray[Shape[Any, Any, 4], Float], thinning_factor: int = 6
) -> NDArray[Shape[2], Float]:
    corner_copy = corner_square.copy()
    width, height = corner_copy.shape[:2]
    bbox = (0, 0, width, height // thinning_factor)

    image = PIL.Image.fromarray(corner_copy)
    patch_inverted = image.crop(bbox).transpose(method=Transpose.FLIP_LEFT_RIGHT)
    return blend_patch_into_image(bbox, image, patch_inverted)


def fill_transparent_corner_horizontal_bottom(
    corner_square: NDArray[Shape[Any, Any, 4], Float], thinning_factor: int = 6
) -> NDArray[Shape[2], Float]:
    corner_copy = corner_square.copy()
    width, height = corner_copy.shape[:2]
    bbox = (0, (thinning_factor - 1) * height // thinning_factor, width, height)
...
def fill_corners(img: NDArray[Shape[2], Float]) -> NDArray[Shape[2], Float]:
    ...
    # left side
    img[:corner_size, :corner_size] = fill_transparent_corner_vertical_left(img[:corner_size, :corner_size])
    img[:corner_size, -corner_size:] = fill_transparent_corner_vertical_right(img[:corner_size, -corner_size:])
    ...
    # top side
    img[:corner_size, :corner_size] = fill_transparent_corner_horizontal_top(img[:corner_size, :corner_size])
    img[-corner_size:, :corner_size] = fill_transparent_corner_horizontal_bottom(img[-corner_size:, :corner_size])
